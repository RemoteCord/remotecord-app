...
impl From<EventType> for SerializableEventType {
    fn from(event_type: EventType) -> Self {
        match event_type {
            EventType::KeyPress(key) => Self {
                event_type: format!("KeyPress({:?})", key),
            },
            EventType::KeyRelease(key) => Self {
                event_type: format!("KeyRelease({:?})", key),
            },
            EventType::MouseMove { x, y } => Self {
                event_type: format!("MouseMove({}, {})", x, y),
            },
            EventType::ButtonPress(button) => Self {
                event_type: format!("ButtonPress({:?})", button),
            },
            EventType::ButtonRelease(button) => Self {
                event_type: format!("ButtonRelease({:?})", button),
            },
            _ => Self {
...
impl From<SerializableEventType> for EventType {
    fn from(serializable: SerializableEventType) -> Self {
        if serializable.event_type.starts_with("MouseMove(") {
            ...
            if let Some(button_str) = serializable
                .event_type
                .strip_prefix("ButtonPress(")
            ...
            if let Some(key_str) = serializable
                .event_type
                .strip_prefix("KeyPress(")
                .and_then(|s| s.strip_suffix(")"))
            {
    ...
    thread::spawn(move || {
        ...
        if let Err(error) = listen(move |event| {
            ...
            if macro_events.is_empty()
                || macro_events.last().unwrap().event_type != event.event_type.clone().into()
            {
...
fn stop_recording(state: State<MacroState>) -> Vec<MacroEvent> {
    ...
    while let Some(last_event) = events.pop() {
        if last_event.event_type.event_type.starts_with("ButtonPress")
            || last_event
                .event_type
                .event_type
                .starts_with("ButtonRelease")
        {
            break; // Remove only the last click event
        } else {
    ...
    thread::spawn(move || {
        for event in macro_events {
            if STOP_FLAG.load(Ordering::SeqCst) {
                println!("Stopping macro playback");
                break;
            }
        ...
        STOP_FLAG.store(true, Ordering::SeqCst); // Ensure the thread stops after the loop
    });
    Ok(())
}
