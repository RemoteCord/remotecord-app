    ...
    tauri::async_runtime::spawn(async move {
        ...
        if let Err(error) = listen(move |event| match event.event_type {
            ...
            EventType::KeyRelease(key) => {
                if let Some(key_str) = get_key_string(key) {
                    (*window_clone).emit("KeyRelease", key_str).unwrap();
                }
            }
            _ => (),
        }) {
...
pub fn get_key_string(key: Key) -> Option<String> {
    match key {
        Key::Alt => Some("Alt".to_string()),
        Key::AltGr => Some("AltGr".to_string()),
        Key::Backspace => Some("'back'".to_string()),
        Key::CapsLock => Some("'caps'".to_string()),
        ...
        Key::MetaLeft => Some("'supr'".to_string()),
        Key::MetaRight => Some("'supr'".to_string()),
